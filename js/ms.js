...
function mouseOverTopMenu(tm){
	...
	//alert(RegExp.$1);
	iname = RegExp.$1
	imgObj.setAttribute("src", imgpref+"img/shared/"+iname+"-hover.gif");
...
function mouseOverSubMenu(subm){
	...
	subIname = RegExp.$1
	subImgObj.setAttribute("src", imgpref+"img/shared/"+subIname+"-hover.gif");
}
...
function mouseOverDD(ddm){
	...
	ddre.exec(activeDD);
	ddIname = RegExp.$1
	ddImgObj.setAttribute("src", imgpref+"img/shared/"+ddIname+"-on.gif");
}
...
function glossary(lang) {
	...
	var gset = 'toolbar=no,location=0,directories=no,status=yes,menubar=0,scrollbars=no,resizable=no,width=690,height=450';
	g_window = window.open('/glossary/index' + langext + '.html', 'glossarywin', gset);
}